...
gecui.form.FeatureTypeFormPanel = function(config) {
	...
	var submit = function() {
		...
		Ext.Ajax.request( {
			method : 'PUT',
			url : gecui.url + 'workspaces/' + data.featureType.namespace.name
					+ '/datastores/' + data.featureType.store.name
					+ '/featuretypes/' + data.featureType.name,
			jsonData : data
		});
	};

	gecui.form.FeatureTypeFormPanel.superclass.constructor.call(this, Ext
			.apply( {
				frame : true,
				defaults : {
					anchor : '95%'
